...
// Puertos, variables
	...
	// LCD
		...
		int LCD_D6 				= 12;
		int LCD_D7 				= 13;
		int LCD_Backlight 		= 7;	// cambiar para vercion 0.1
		LiquidCrystal lcd (LCD_RS, LCD_E, LCD_D4, LCD_D5, LCD_D6, LCD_D7);  //LCD setup
		byte Back_Light 		= 0;	// porcentaje de back light
	// Versiones
		const byte Firm_Ver_Ent = 0;
		const byte Firm_Ver_Dec = 8;
...
void Init_EEPROM()
	{
		// LCD
			LCD_Backlight = EEPROM.read(513);  	// valor entre 0 y 100
	}

void LCD_Back_Light(byte save)
	{
		analogWrite(LCD_Backlight, Back_Light);	// PWM de back light
		if (save == 1)
			{
				EEPROM.write(LCD_Backlight, 513);
			}
	}
		...
		inicio:
			...
			// configuracion de cursor	
				...
				// Acciones
					...
					Cursor_Conf[1][11]  = 1; 	// About
					Cursor_Conf[1][1]   = 1; 	// Config
			// navegar
				GUI_Navegar(0, 0);
			// Acciones
				// Unitary
...
void GUI_Config()
	{
		iniciar:
			// LCD
				...
				lcd.print ("Config:");
				lcd.setCursor (0, 2);
				lcd.print ("LCD Back Light: 000");
				lcd.setCursor (0, 3);
				lcd.print ("000-255         Exit");
				Numerico_Write (LCD_Backlight, 16, 2);
			// Cursor
		...
		Siguiente:	
			// configuracion de cursor		
				Cursor_Conf_Clear();		// limpiar array
			// Acciones
				Cursor_Conf[2][15]   = 1;	// LCD Back Light
					...
					if (LCD_Col_Pos == 15 && LCD_Row_Pos == 2)
						{
							...
							if (LCD_Backlight > 255)
								{
									Num_Val = 255;
									Numerico_Write (Num_Val, 16, 2);
								}
							LCD_Backlight = Num_Val;
							LCD_Back_Light(1);
						}
				// Exit
